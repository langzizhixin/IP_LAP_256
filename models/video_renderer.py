...
class SPADELayer(torch.nn.Module):
    def __init__(self, input_channel, modulation_channel, hidden_size=256, kernel_size=3, stride=1, padding=1):
        ...
        self.conv1 = torch.nn.Conv2d(modulation_channel, hidden_size, kernel_size=kernel_size, stride=stride, padding=padding)
    ...
    def forward(self, input, modulations):
        norm = self.instance_norm(input)  # (B, C, H, W)

        # 确保 modulations 的批量大小与 input 一致
        if modulations.size(0) < input.size(0):
            modulations = modulations.repeat(input.size(0) // modulations.size(0), 1, 1, 1)

        conv_out = self.conv1(modulations)  # (B, hidden_size, H', W')
        gamma = self.gamma(conv_out)  # (B, C, H', W')
        beta = self.beta(conv_out)    # (B, C, H', W')

        # 调整 gamma 和 beta 的尺寸以匹配 norm
        gamma = F.interpolate(gamma, size=norm.size()[2:], mode='bilinear', align_corners=False)
        beta = F.interpolate(beta, size=norm.size()[2:], mode='bilinear', align_corners=False)
...
class DenseFlowNetwork(torch.nn.Module):
    def __init__(self, num_channel=6, num_channel_modulation=3*5, hidden_size=128):
    ...
    def forward(self, ref_N_frame_img, ref_N_frame_sketch, T_driving_sketch):
        ...
        for ref_idx in range(ref_N):
            ...
            h1 = self.conv1_relu(self.conv1_bn(self.conv1(flow_module_input)))  # (32, 256, 256)
            h2 = self.conv2_relu(self.conv2_bn(self.conv2(h1)))  # (256, 128, 128)

            downsample_64 = downsample(driving_sketch, (128, 128))  # driving_sketch: (B*T, 3, H, W)

            spade_layer = self.spade_layer_1(h2, downsample_64)  # (256, 128, 128)
            spade_layer = self.spade_layer_2(spade_layer, downsample_64)  # (256, 128, 128)

            spade_layer = self.pixel_shuffle_1(spade_layer)  # (64, 256, 256)

            spade_layer = self.spade_layer_4(spade_layer, driving_sketch)  # (64, 256, 256)

            output_flow = self.conv_4(spade_layer)  # (B*T, 2, 256, 256)
            output_weight = self.conv_5(spade_layer)  # (B*T, 1, 256, 256)

            deformation = convert_flow_to_deformation(output_flow)
            wrapped_h1 = warping(h1, deformation)  # (32, 256, 256)
            wrapped_h2 = warping(h2, deformation)  # (256, 128, 128)
            wrapped_ref = warping(ref_img, deformation)  # (3, 256, 256)

            softmax_denominator += output_weight
            wrapped_h1_sum += wrapped_h1 * output_weight
            wrapped_h2_sum += wrapped_h2 * downsample(output_weight, (128, 128))
            wrapped_ref_sum += wrapped_ref * output_weight

        softmax_denominator += 0.00001
        wrapped_h1_sum = wrapped_h1_sum / softmax_denominator
        wrapped_h2_sum = wrapped_h2_sum / downsample(softmax_denominator, (128, 128))
...
class Renderer(torch.nn.Module):
    ...
    def forward(self, face_frame_img, target_sketches, ref_N_frame_img, ref_N_frame_sketch, audio_mels): #T=1
        ...
        """检查 Perceptual Loss:感知损失（Perceptual Loss）部分默认使用 VGG 网络，其期望输入尺寸为 224×224，与目标尺寸 256×256 不一致。如果使用感知损失，需要在输入到 VGG 网络时进行一次缩放：
