...
parser.add_argument('--input', '--input_template_video', type=str, default='./test/555.mp4')
parser.add_argument('--audio', type=str, default='./test/555.wav')
...
with mp_face_mesh.FaceMesh(static_image_mode=True, max_num_faces=1, refine_landmarks=True,
    ...
    for frame_idx, full_frame in enumerate(ori_background_frames):
        ...
        ###########   这个参数需要和 preprocess_video.py  第 252 行 ########plus 5 pixel to size###### 参数对应
        # plus_pixel = 25   #默认参数是25
...
for frame_idx in range(ref_img_full_face_landmarks.shape[0]):  # N
    ...
    drawn_sketech = cv2.resize(drawn_sketech, (img_size, img_size))  # (256, 256, 3)
...
for batch_idx, batch_start_idx in tqdm(enumerate(range(0, input_mel_chunks_len - 2, 1)),
    ...
    T_target_sketches = torch.stack(T_target_sketches, dim=0).permute(0, 3, 1, 2)  # (T,3,256, 256)
    target_sketches = T_target_sketches.unsqueeze(0).cuda()  # (1,T,3,256, 256)
